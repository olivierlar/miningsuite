...
%%
    methods
        function s = Envelope(data,srate,sstart,ssize,name)
            if nargin<5
                name = 'Envelope';
            end
            s = s@sig.Signal(data,name,'','','',0,1,srate,sstart,ssize);
        end
        %%
...
function options = initoptions
    ...
    options.fsize.default.value = .1;
    options.fhop.default.value = .1;
...
function out = main(x,option,postoption)
    if isnumeric(x) % MIDI Note matrix
        ...
        %%
        d = sig.data(g,{'sample'});
        obj = sig.Envelope(d,ndivs,0,length(g));
    elseif isa(x{1},'sig.Envelope')
        obj = x{1};
    else
        d = sig.compute(@routine,x{1}.Ydata,x{1}.Srate,option);
        
        obj = sig.Envelope(d,x{1}.Srate,x{1}.Sstart,x{1}.Ssize);
    end
